...
fn sdk_workflow_examples_are_present_and_documented() {
    ...
    assert!(readme.contains("cargo run -p quic_tunnel_sdk --example sdk_mock_workflow"));
    assert!(readme.contains("cargo run -p quic_tunnel_sdk --example sdk_live_workflow"));
}
